...
When `key` is present, the library wraps the resolver in `Effect.request` + `RequestResolver` so the walker automatically batches concurrent fetches at each depth level — 50 Users needing Posts becomes 1 batched query, not 50. The `resolve` function receives the requested keys and returns a `ReadonlyMap<K, Type>`. A resolver failure fails the whole batch (and therefore the operation); a missing key is a resolver contract violation and dies as a defect. Batching is effect-only; stream fields don't batch (different primitive).
...
**`MissingOnVariant` wire shape.** The walker writes plain `undefined` for a selected field absent from the matched variant, and the response codec makes any undefined-admitting slot an _optional key_ — struct encode drops undefined-valued keys, so a required key could not decode its own encode output. On the wire, absent-on-variant is therefore key omission; in-process it is `undefined`. This still conflates "absent on this variant" with "resolved to undefined"; a wire-visible marker remains deferred until an adapter needs the distinction.
