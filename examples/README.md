...
- `http-dispatch.ts` — GraphQL-like dynamic HTTP gateway over plain JSON. The
  client sends runtime `args` and `select`, and the server validates them with
  `domain.dispatch(...)`. This is useful when you want selectable responses over
  ordinary HTTP without GraphQL's parser, schema language, introspection, or
  execution semantics.
